...
from project4.view.user_input import get_values_from_user
...
def run_genetic_algoritm():
    ...
    for item in chromossomeBinaryList:
        newChromossome = Chromossome(
            format_binary_code(item, inputResult.chromossomeSize), actualGeneration
        )
        binX, binY = (
            newChromossome.geneticCode[: int(len(newChromossome.geneticCode) / 2)],
            newChromossome.geneticCode[int(len(newChromossome.geneticCode) / 2) :],
        )
    ...
    while actualGeneration < inputResult.quantityOfGeneration:
        ...
        if inputResult.methodOfSelection == 1:
            crossoverChromossomesGeneticCodes = make_crossover(
                inputResult, generation=actualGeneration
            )
            # Need to create chromossome and add on the new list
            for newChromossome in crossoverChromossomesGeneticCodes:

                binX, binY = (
                    newChromossome.geneticCode[
                        : int(len(newChromossome.geneticCode) / 2)
                    ],
                    newChromossome.geneticCode[
                        int(len(newChromossome.geneticCode) / 2) :
                    ],
            ...
            # crossover tournment
            crossoverChromossomesGeneticCodes = run_tournment_selection(
                inputResult, generation=actualGeneration
            )
            for item in crossoverChromossomesGeneticCodes:

                binX, binY = (
                    item.geneticCode[: int(len(item.geneticCode) / 2)],
                    item.geneticCode[int(len(item.geneticCode) / 2) :],
                )
                realX = inputResult.getConvertionFromBinaryToRealX(binX)
                realY = inputResult.getConvertionFromBinaryToRealY(binY)
        ...
        bestChromosomeGeneration = get_best_chromossome(
            inputResult.currentChromossomeList
        )
    ...
    for bestChromosome in bestChromossomeList:
        print(
            ...
            + bestChromosome.geneticCode
            + "\tGeneration: "
            + str(bestChromosome.generation)
            + "\tFitness: "
