...
def verify_list(chromosomes_list):
    ...
    if not new_verify_list(new_finally_list):
        print(f"Chr: {new_finally_list}")
...
def run_tournament_selection(genetic_algoritm, generation):
    ...
    for i in range(int(genetic_algoritm.population_size / 2)):
        ...
        new_chromosomes = make_crossover_ox(
            tournament_result[0].genetic_code, tournament_result[1].genetic_code
        )
