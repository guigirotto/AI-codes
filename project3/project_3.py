...
def run_genetic_algoritm_2():
    ...
    while actual_generation < input_result.quantity_of_generation:
        ...
        if input_result.method_of_selection == 1:
            ...
            if input_result.elitism_size > 0:
                ...
                for i in range(len(elitsm_list)):
                    new_chromosome_list[i] = Chromosome(
                        elitsm_list[i].genetic_code,
                        actual_generation,
                        elitsm_list[i].fitness,
                    )
        ...
        for index, item in enumerate(new_chromosome_list):
            #  Keep the elitism without mutations
            new_chromosome_list[index].genetic_code = verify_list(item.genetic_code)
            if not (index < input_result.elitism_size):
        ...
        best_chromosome_list.append(
            Chromosome(
                best_chromosome.genetic_code, actual_generation, best_chromosome.fitness
            )
        )
    ...
    # i = 0
    # for item in best_chromosome_list:
    #     i+=1
    #     print(f"\tGen: {i} \t Valid: {new_verify_list(item.genetic_code)}")
