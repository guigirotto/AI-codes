...
def get_values_from_user():
    ...
    method_of_selection = (input('Inform a selection method to the algoritm \n 1 - Roullete \n 2 - Tournment \n'))
    if not (method_of_selection.isdigit() and (method_of_selection == '1' or method_of_selection == '2')):
    ...
    result_of_check = check_if_user_type_only_digits(chromosome_size, population_size, crossing_probability,
                                                     mutation_probability, method_of_selection, elitism_size,
                                                     quantity_of_crossing, quantity_of_generation)
    if result_of_check:
        genetic_algoritm = GeneticAlgoritm(chromosome_size, population_size, crossing_probability, mutation_probability,
                                           method_of_selection, elitism_size, quantity_of_crossing,
                                           quantity_of_generation)
        if genetic_algoritm.method_of_selection == 2:
            genetic_algoritm.settournament_size(tournament_size)
